...
DEF_CMD_(OUT,    9,   1, { outputFromRegister(&cpu, command); EIP+=2; })
DEF_CMD_(IN,    10,   1, {    inputToRegister(&cpu, command); EIP+=2; })
...
DEF_CMD_(JBE,   15,  -1, { double a = stack_pop(&cpu.stack); double b = stack_pop(&cpu.stack); printf("\n a = %f, b = %f\n",a,b); if(a >= b) EIP = (int)command[EIP + 1]; else EIP += 2; stack_push(&cpu.stack, b); stack_push(&cpu.stack, a); })
...
DEF_CMD_(CALL,  18,  -1, { stack_push(&cpu.callStack, EIP  + 2); EIP =(int) command[EIP + 1]; })
